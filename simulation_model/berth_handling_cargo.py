...
class BerthHandlingCargo(ActivityHandler):
    ...
    def _get_duration(self, berth: 'Berth', rs) -> dt.timedelta:
        """
        Estimate the number of simultaneously deployed quay cranes from vessel
        length, using roughly one crane per 50 metres of LOA.  Productivity per
        crane remains 45 TEU/hour.
        """
        ...
        qc_count = max(1, round(vessel_class.loa / 50))
        discharging_teu = sum(
