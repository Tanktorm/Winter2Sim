...
def find_available_port(start_port: int = DASHBOARD_START_PORT, attempts: int = 50) -> int:
    """Return the first free loopback port at or above start_port."""
    for port in range(start_port, start_port + attempts):
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            try:
                sock.bind((DASHBOARD_HOST, port))
            except OSError:
                continue
            return port
    raise OSError(
        f"No free dashboard port found from {start_port} "
        f"to {start_port + attempts - 1}."
    )
...
def launch_dashboard() -> None:
    """Run dashboard/serve_gui.py in the background on an available port."""
    ...
    arguments = [sys.executable, str(DASHBOARD_SCRIPT), str(port), "--no-open"]
    ...
    try:
        ...
        print(
            ...
            f"{sys.executable} {DASHBOARD_SCRIPT} {port}",
            file=sys.stderr,
        )
